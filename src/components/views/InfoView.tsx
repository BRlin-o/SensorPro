...
export function InfoView({ isLandscape }: InfoViewProps) {
    return (
        <div className={`${isLandscape ? 'flex flex-row gap-4 items-start' : 'flex flex-col gap-6'}`}>
            {/* 關於本 App 的漸層標頭卡片 */}
            <Card className={`bg-gradient-to-br from-blue-600 to-indigo-700 text-white border-none
                ${isLandscape ? 'flex-1 min-w-0' : ''}`}
            >
                <h2 className={`font-bold mb-2 ${isLandscape ? 'text-lg' : 'text-xl'}`}>關於傳感器 App</h2>
